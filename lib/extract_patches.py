...
def get_data_training(train_imgs_original, train_groundTruth,
					  patch_height, patch_width,
					  N_subimgs, inside_FOV, patches):
	...
	visualize(group_images(train_imgs[100:120,:,:,:],5),'imgs_train')#.show()  #check original imgs train
	...
	visualize(group_images(train_imgs[100:120,:,:,:],5),'imgs_train_random')
	visualize(group_images(train_groundTruth[100:120,:,:,:],5),'gTruths_train_random')
	...
	print "train ground truths range (min-max): " + str(np.min(train_groundTruth)) + ' - ' + str(np.max(train_groundTruth))

	if patches == True:
		# Extract the TRAINING patches from the full images
		patches_imgs_train, patches_groundTruths_train = extract_random(train_imgs,train_groundTruth,patch_height,patch_width,N_subimgs,inside_FOV)
		...
		print "train PATCHES ground truths shape: " + str(patches_groundTruths_train.shape)
		print "train PATCHES ground truths range (min-max): " + str(np.min(patches_groundTruths_train)) + ' - ' + str(np.max(patches_groundTruths_train))
...
def get_data_testing(test_imgs_original, test_groudTruth, Imgs_to_test, patch_height, patch_width):
	...
	print "train images range (min-max): " + str(np.min(test_imgs)) + ' - ' + str(np.max(test_imgs))
	print "train ground truths shape:" + str(test_groundTruths.shape)
	print "train ground truths range (min-max): " + str(np.min(test_groundTruths)) + ' - ' + str(np.max(test_groundTruths))

	#extract the TEST patches from the full images
	patches_imgs_test = extract_ordered(test_imgs,patch_height,patch_width)
	patches_groundTruths_test = extract_ordered(test_groundTruths,patch_height,patch_width)
	...
	print "test PATCHES ground truths range (min-max): " + str(np.min(patches_groundTruths_test)) + ' - ' + str(np.max(patches_groundTruths_test))
...
def get_data_testing_overlap(test_imgs_original,
							test_groudTruth,
							Imgs_to_test,
							patch_height, patch_width,
							stride_height, stride_width, patches):
	### test
	test_imgs_original = load_hdf5(test_imgs_original)
	test_imgs_groundTruth = load_hdf5(test_groudTruth)

	test_imgs = my_PreProc(test_imgs_original)
	test_imgs_groundTruth = test_imgs_groundTruth/255.

	#extend both images and masks so they can be divided exactly by the patches dimensions
	# test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
	# test_imgs_groundTruth = test_imgs_groundTruth[0:Imgs_to_test,:,:,:]
	# test_imgs = paint_border_overlap(test_imgs, patch_height, patch_width, stride_height, stride_width)

	#check masks are within 0-1
	assert(np.max(test_imgs_groundTruth)==1  and np.min(test_imgs_groundTruth)==0)

	print "\ntest images shape:"
	print test_imgs.shape
	print "\ntest mask shape:"
	print test_imgs_groundTruth.shape
	...
	if patches == True:
		...
		return patches_imgs_test, test_imgs.shape[2], test_imgs.shape[3], test_imgs_groundTruth
		#return patches_imgs_test, test_imgs.shape[2], test_imgs.shape[3]
	else:
		return test_imgs, test_imgs.shape[2], test_imgs.shape[3], test_imgs_groundTruth


# data consinstency check - checks if imgs shape == masks shape
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
	print "FULL IMG SHAPE", full_imgs.shape[0]
	...
	# THEO assert (full_masks.shape[1]==1)   #masks only black and white
...
def pred_only_FOV(data_imgs,data_masks):
	...
	# THEO assert (data_imgs.shape[1]==1 and data_masks.shape[1]==1)  #check the channel is 1
...
# 	# THEO assert (DRIVE_masks.shape[1]==1)  #DRIVE masks is black and white
